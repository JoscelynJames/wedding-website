...
class DataView extends Component {
  ...
  componentDidMount() {
    api.getGuests().then(res => this.setState({ guests: res.data }));

    api.getSongs().then(res => this.setState({ songs: res.data }));
  }
            ...
            {this.state.guests.map((guest, index) => {
              return (
                <TableRow
                  key={index}
                  className={
                    guest.attending ? styles.isAttending : styles.notAttending
                  }
                >
